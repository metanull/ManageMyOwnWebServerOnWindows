...
Describe "Testing public module function Stop-WorkerVite" -Tag "UnitTest" {
    Context "General context" {
        ...
        It "Stop-WorkerVite should return true when no server is running" {
            Mock Test-DevPort { return $false }  # Port is FREE (no server running)
            
            $Result = Stop-WorkerVite -Port 5173
            $Result | Should -Be $true
        }

        It "Stop-WorkerVite should stop Vite process successfully" {
            Mock Test-DevPort { 
                param([int]$Port)
        ...
        It "Stop-WorkerVite should stop Vite process with Force" {
            Mock Test-DevPort { 
                param([int]$Port)
            ...
            $Result = Stop-WorkerVite -Port 5173 -Force
            $Result | Should -Be $true
            Should -Invoke Read-Host -Exactly 0 -Scope It  # No confirmation with Force
        }

        It "Stop-WorkerVite should handle non-Vite process without Force" {
        ...
        It "Stop-WorkerVite should handle no processes found on busy port" {
            Mock Test-DevPort { return $true }  # Port is busy
            Mock Get-NetTCPConnection {
                param([int]$LocalPort, [string]$ErrorAction)
        ...
        It "Stop-WorkerVite should handle process stop failure" {
        ...
        It "Stop-WorkerVite should handle user declining to stop process" {
            Mock Test-DevPort { return $true }  # Port is busy and stays busy
            Mock Read-Host {
                param([string]$Prompt)

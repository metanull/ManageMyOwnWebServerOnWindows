Describe "Get-Job" -Tag "Functional","BeforeBuild" {
    ...
    Context "When the function is called" {
        ...
        It "Should not throw an exception when Pipeline Id is given and Stage Index is given but no Job Index is given" {
            $TestData.Pipelines | Foreach-Object {
                $Pipeline = $_
                $Pipeline.Stages | Foreach-Object {
                    $Stage = $_
        ...
        It "Should return all jobs when Pipeline Id is provided and Stage Index is given but no Job Index is given" {
            $TestData.Pipelines | Foreach-Object {
                $Pipeline = $_
                $Pipeline.Stages | Foreach-Object {
                    $Stage = $_
                    $Result = Invoke-ModuleFunctionStub -Id $Pipeline.Id -Stage $Stage.Index
                    $Result.Count | Should -Be $Pipelines.Stages.Jobs.Count
                }
            }
        }
        It "Should return the expected data when the Pipeline Id is provided and Stage Index is given as well as the Job Index" {
            $TestData.Pipelines | Foreach-Object {
                $Pipeline = $_
                $Pipeline.Stages | Foreach-Object {
                    $Stage = $_

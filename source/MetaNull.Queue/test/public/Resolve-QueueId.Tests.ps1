Describe "Resolve-QueueId" -Tag "Functional","BeforeBuild" {
    ...
    Context "When the function is called" {
        ...
        It "Completion Should return the expected Id when exact value provided" {
            $TestData | Foreach-Object {
                $CurrentTest = $_
                $Result = Invoke-ModuleFunctionStub -PartialId $CurrentTest.Queue.Id
                $Result | Should -Be $CurrentTest.Queue.Id
            }
        }
        It "Completion Should return the expected Id when partial value provided" {
            $TestData | Foreach-Object {
                $CurrentTest = $_
                $Result = Invoke-ModuleFunctionStub -PartialId ($CurrentTest.Queue.Id -replace '...$','*')
                $Result | Should -Be $CurrentTest.Queue.Id
            }
        }
